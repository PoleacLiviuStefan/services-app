...
const initializeZoom = useCallback(async () => {
  ...
  try {
    ...
    // 7. Join session - COMPLETE FIX WITH DEBUGGING
    addLog("🔗 Joining session:", info.sessionName);

    // ✅ DEBUGGING COMPLET AL TOKEN-ULUI
    try {
      const tokenParts = info.token.split('.');
      if (tokenParts.length !== 3) {
        throw new Error(`Invalid token format: expected 3 parts, got ${tokenParts.length}`);
      }
      
      addLog("🔍 Complete token analysis:", {
        tokenLength: info.token.length,
        tokenParts: tokenParts.length,
        header: JSON.parse(atob(tokenParts[0])),
        signature: tokenParts[2].substring(0, 10) + "..."
      });

      // ✅ AFIȘARE COMPLETĂ A PAYLOAD-ULUI
      const payload = JSON.parse(atob(tokenParts[1]));
      addLog("🔍 Complete token payload:", payload);

      // Verificări specifice Zoom
      const requiredFields = ['iss', 'exp', 'app_key', 'tpc', 'user_identity', 'role_type', 'version'];
      ...
      // ✅ VERIFICARE SPECIFICĂ ZOOM SDK
      addLog("🔍 Zoom token field validation:", {
        iss: { type: typeof payload.iss, value: payload.iss, valid: typeof payload.iss === 'string' && payload.iss.length > 0 },
        exp: { type: typeof payload.exp, value: payload.exp, valid: typeof payload.exp === 'number' && payload.exp > Date.now() / 1000 },
        app_key: { type: typeof payload.app_key, value: payload.app_key, valid: typeof payload.app_key === 'string' && payload.app_key.length > 0 },
        tpc: { type: typeof payload.tpc, value: payload.tpc, valid: typeof payload.tpc === 'string' && payload.tpc === info.sessionName },
        user_identity: { type: typeof payload.user_identity, value: payload.user_identity, valid: typeof payload.user_identity === 'string' && payload.user_identity.length > 0 },
        role_type: { type: typeof payload.role_type, value: payload.role_type, valid: typeof payload.role_type === 'number' && [0, 1].includes(payload.role_type) },
        version: { type: typeof payload.version, value: payload.version, valid: typeof payload.version === 'number' }
      });
      ...
      if (expiresIn <= 0) {
        throw new Error(`Token expired ${Math.abs(expiresIn)} seconds ago`);
      }

      if (expiresIn < 60) {
        addLog("⚠️ Token expires very soon:", expiresIn + " seconds");
      }

      // ✅ PREPARE JOIN PARAMS - EXACT ZOOM SDK FORMAT
      const userName = String(auth?.user?.name || auth?.user?.email || `User_${Date.now()}`).trim();

      addLog("🔍 Preparing join with exact types:", {
        signature: { type: typeof info.token, value: info.token.substring(0, 20) + "..." },
        topic: { type: typeof info.sessionName, value: info.sessionName },
